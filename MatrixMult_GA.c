...
/*
 * Check to see if inversion is correct. This function makes extensive
 * use of transposes because the underlying GA_Dgemm routine has been written
 * assuming Fortran-style indexing and memory layout.
 */
#define TOLERANCE 0.1
void verify(int g_a, int g_b, int g_c, int *lo, int *hi, int *ld) {

    double rchk, alpha=1.0, beta=0.0;
    int g_chk, g_trns, me=GA_Nodeid();

    g_chk = GA_Duplicate(g_a, "array Check");
    g_trns = GA_Duplicate(g_a, "array Check");
    if(!g_chk || !g_trns) GA_Error("duplicate failed",NDIM);
    GA_Sync();

    GA_Dgemm('t', 't', TOTALELEMS, TOTALELEMS, TOTALELEMS, 1.0, g_a,
             g_b, 0.0, g_chk);

    GA_Sync();
    GA_Transpose(g_chk, g_trns);
    
    alpha=1.0, beta=-1.0;
    GA_Add(&alpha, g_c, &beta, g_trns, g_chk);
